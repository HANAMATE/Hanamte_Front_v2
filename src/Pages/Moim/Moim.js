...
const MoimList = () => {
    ...
    return (
        ...
        {moims !== [] ? moims.map((moim) => (
            <div onClick={()=>moimCardClick(moim.walletId)} style={{ cursor: "pointer" }}>
          ...
          <HomeCard key={-1} title="모임통장 만들기" bgcolor = "linear-gradient(-45deg, #5f3084, #3f3084)"
          tcolor="#ccc" dcolor="#fbfbfb"
            description = {"목표금액도 정하고 모임통장 만들러가기!"} icon={WalletPurple} 
             />
             </div>
            </div>
            </div>
